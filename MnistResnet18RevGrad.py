...
class SmallStemResNet18(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc(features)      # -> [B, num_classes]
        domain=self.domainfc(features)
        
        return x, domain
    
    # step scheduler for lambda by incrementing the epoch count
    def step_scheduler(self):
        for m in self.domainfc.modules():
            if hasattr(m, "step_scheduler"):
                m.step_scheduler()

    
    # clip weights to improve stability
